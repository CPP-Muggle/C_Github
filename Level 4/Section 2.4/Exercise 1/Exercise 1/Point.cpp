...
Point::Point(const Point &p):Shpae(const source)//copy constructor
{
	m_x = p.X();
	m_y = p.Y();
}
...
Point& Point::operator = (const Point& source) : Shape::operator = (const Shape source)   // Assignment operator.
{
	m_x = source.m_x;
	m_y = source.m_y;
	return *this;
