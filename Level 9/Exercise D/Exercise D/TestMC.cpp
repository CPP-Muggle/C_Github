...
//I stick to the formula given by pdf although I think it is not right
double SD(vector<double> S, double r, double T){
	double sum_square = 0;
	double square_sum = 0;
	double SD;
	int M = S.size();
	for (int i = 0; i < M; i++){
		sum_square = sum_square + pow(S[i], 2);
		square_sum = square_sum + S[i];
	}
	square_sum = pow(square_sum, 2);
	SD = sqrt(sum_square - 1.0 / M*square_sum)*exp(-2.0 * r*T) / (M - 1.0);
	return SD;
}
...
int main()
{
	...
	{ // Calculate a path at each iteration
		...
		C.push_back(tmp);//store the simulation option price in C
		price += (tmp)/double(NSim);
	}
	std::cout << "SD is: " << SD(C, myOption.r, myOption.T) << endl;
	std::cout << "SE is: " << SD(C, myOption.r, myOption.T) / sqrt(NSim) << endl;
